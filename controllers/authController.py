...
from utils.jwt import (signToken, checkToken)

from sql.userQueries import (insertUser, getUserWithId,
...
def login():
    ...
    if ("email" not in data or "password" not in data):
        return abort(400, 'please provide an email and password')

    # 2) Check if user exists
    cnx = use_db()
    db = cnx.cursor()
    db.execute(getUserWithEmail(data["email"]))
    result = db.fetchall()
    if result == ():
        return abort(400, 'no user with this email')

    user = result[0]

    # 3) check password is correct
    if not comparePasswords(user["password"], data["password"]):
        return abort(400, 'the password you enterd is wrong, please try again')
...
def forgotPassword():
    ...
    result = db.fetchall()
    if result == ():
        abort(400, 'no user with this email')
    ...
    try:
        ...
        res(200, {"msg": "mail send successfully"})
    except:
        # db.execute(updateUserPassResetData(
        #     user["user_id"], "NULL", "NULL"))
        # cnx.commit()
        abort(400, 'an error during sending the email, please try again')
...
def resetPassword(token):
    ...
    db.execute(getUserByResetToken(cryptedToken, now))
    user = db.fetchone()
    # 2) If token has not expired, and there is user, set the new password
    if "user_id" not in user:
        abort(400, "Token is invalid or has expired")
    # 3) hash an Update password, changedPasswordAt property for the user
    data = req.get_json()
    if ("password" not in data):
        return abort(400, 'please provide an password')

    hashed = hashPassword(data["password"])

    nowDate = int(round(time.time() * 1000))
    db.execute(updateResetPassword(user["user_id"], hashed, nowDate))
    cnx.commit()
...
def updatePassword():
    ...
    if "currentPassword" not in data or "newPassword" not in data:
        abort(400, "please enter the needed data!")
    ...
    if "name" not in user:
        abort(400, "cant find this user")

    if not comparePasswords(user["password"], data["currentPassword"]):
        abort(401, 'Your current password is wrong')

    hashed = hashPassword(data["newPassword"])
    nowDate = int(round(time.time() * 1000))

    db.execute(updateResetPassword(req.user["user_id"], hashed, nowDate))
    cnx.commit()
...
def protect():
    ...
    if "Authorization" not in req.headers:
        abort(401, 'you are not logged in, please login')
    ...
    try:
        ...
        abort(401, 'wrong or expired token.')

    # 3) Check if user still exists
    cnx = use_db()
    db = cnx.cursor()
    db.execute(getUserWithId(decoded["id"]))
    user = db.fetchone()
    if "user_id" not in user:
        abort(401, 'The user belonging to this token does no longer exist.')
